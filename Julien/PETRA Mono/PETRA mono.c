...
int main(int argc, char *argv[])
{
	...
	//Selection critere de tri
	printf("\nSysteme PETRA pret\n\nVeuillez choisir le critere de tri de piece\n");
	printf("1) Slot et Corner\n");
	printf("2) Slot uniquement\n");
	printf("3) Corner uniquement\n==>");
	read(1, &buf, 1);
	if(buf == '1')
	{
		selectslot = 1;
    ...
    while(1)
    {
	    ...
	    while(!u_capt.capt.DE)
	    {
			...
			if(u_capt.capt.L1 && selectcorner)
				badcorner = 1;

			if(selectslot && !badslot)
				printf("Slot OK\n");
			else
				if(selectslot && badslot)
					printf("Slot pas OK\n");

			if(selectcorner && !badcorner)
				printf("Corner OK\n");
			else
				if(selectcorner && badcorner)
					printf("Corner pas OK\n");

			//Sequence Mauvaise piece
			if(badslot || badcorner)
			{
